...
!function(a, b) {
    ...
    var c, d = a.SpeechRecognition || a.webkitSpeechRecognition;
    ...
    var e, f, g = [], h = {
        ...
        for (var b, c = 0; c < a.length; c++) {
            b = a[c].trim(),
            k && w("Discurso reconocido: %c" + b, l);
            for (var d = 0, e = g.length; d < e; d++) {
                var f = g[d]
                  , i = f.command.exec(b);
                if (i) {
                    var j = i.slice(1);
                    return k && (w("Comando reconocido: %c" + f.originalPhrase, l),
                    j.length && w("with parameters", j)),
                    f.callback.apply(this, j),
                    void u(h.resultMatch, b, f.originalPhrase, a)
                }
            }
        }
        u(h.resultNoMatch, a)
    ...
    return c = {
        init: function(l) {
            ...
            e.onerror = function(a) {
                ...
                case "service-not-allowed":
                    f = !1,
                    (new Date).getTime() - i < 200 ? u(h.errorPermissionBlocked, a) : u(h.errorPermissionDenied, a)
                }
            }
            ,
            e.onend = function() {
                ...
                f) {
                    ...
                    j += 1,
                    j % 10 === 0 && k && w("El reconocimiento de voz se detiene y se inicia repetidamente. See http://is.gd/annyang_restarts for tips."),
                    a < 1e3 ? setTimeout(function() {
                        c.start({
                            paused: m
                        })
                    }, 1e3 - a) : c.start({
            ...
            e.onresult = function(a) {
                if (m)
                    return k && w("Speech heard, but annyang is paused"),
                    !1;
            ...
            ,
            o && (g = []),
            l.length && this.addCommands(l)
        },
        start: function(a) {
            x(),
            a = a || {},
            m = a.paused !== b && !!a.paused,
            f = a.autoRestart === b || !!a.autoRestart,
            a.continuous !== b && (e.continuous = !!a.continuous),
            i = (new Date).getTime();
            try {
                e.start()
            } catch (a) {
        ...
        trigger: function(a) {
            return c.isListening() ? (Array.isArray(a) || (a = [a]),
            void z(a)) : void (k && w(n ? "Speech heard, but annyang is paused" : "Cannot trigger while annyang is aborted"))
        }
    }
});
